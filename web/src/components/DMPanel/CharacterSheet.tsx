...
export function CharacterSheet({ stats, schema, displayName, deathState, isAlive }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      {Object.entries(stats).map(([name, value]) => {
        ...
        // Determine bar colors: schema bar_color > known name fallback
        let barColors: { fg: string; bg: string } | null = null;
        if (isNumber && max != null) {
          if (def?.is_resource_bar && def.bar_color) {
            // Use hex color from schema; derive a muted bg from it
